...
/**
 * 抽象的WEB请求实现<br>
 * 实现类如果需要变更UA的话，可调用{@link #setUserAgent(String)}
 * 
 * @author floydchenxf
 */
public abstract class AbstractWebUtils {
	...
	static {
		if (Build.VERSION.SDK_INT < 9) {
			Log.w(TAG,
					"HTTP connection reuse which was buggy pre-froyo,disabled.");
	...
	public Response doPost(String url, Map<String, String> params,
			List<Header> headers, int connectTimeout, int readTimeout)
			throws IOException {
	 ...
	 */
	public Response doPost(String url, Map<String, String> params,
			List<Header> headers, String charset, int connectTimeout,
	 ...
	 */
	public Response doPost(String url, Map<String, String> params,
			List<Header> headers, Map<String, FileItem> fileParams,
	...
	public Response doPost(String url, String ctype, byte[] content,
		...
		try {
			try {
				conn = getConnection(new URL(url), HttpMethod.POST.name(),
						ctype);
	...
	private void addHeaders(List<Header> headers, HttpURLConnection conn) {
		if (headers != null) {
			for (Header head : headers) {
				conn.addRequestProperty(head.getName(), head.getValue());
			}
	...
	public Response doGet(String url, Map<String, String> params,
			int connectTimeout, int readTimeout) throws IOException {
		return doGet(url, params, null, DEFAULT_CHARSET, connectTimeout,
				readTimeout);
	}
	 ...
	 */
	public Response doGet(String url, Map<String, String> params,
			List<Header> header, String charset, int connectTimeout,
			int readTimeout) throws IOException {
		HttpURLConnection conn = null;
	 ...
	 */
	public String buildGetUrlForString(String url, Map<String, String> params,
	...
	protected Response getResponse(HttpURLConnection conn) throws IOException {
		...
		if (es == null) {
			...
			response.setContent(getStreamAsBytes(input));
		} else { // 包含http错误
			if (isGzip) {
				es = new GZIPInputStream(es);
			}
			byte[] msg = getStreamAsBytes(es);
	...
	private byte[] getStreamAsBytes(InputStream stream) throws IOException {
		try {
